...
/**
 * FIELD-R19 village foundation repair.
 *
 * <p>R18 could extend cobblestone up to 24 blocks below a house on a steep
 * island. In the field this reads as a duplicated stone wall. R19 caps the
 * visible support depth at 12 blocks and uses terrain-matched foundation
 * material instead of a universal cobblestone column.</p>
 */
public final class NeverOverworldVillageFoundationR19 {
    static final int MAX_SUPPORT_DEPTH = 12;
    static final int PIECE_FLOOR_SCAN = 4;
    static final int STRUCTURE_EVIDENCE_SCAN = 8;

    private NeverOverworldVillageFoundationR19() {}
    ...
    public static int apply(final WorldGenLevel level, final StructureStart start, final ChunkPos owner) {
        ...
        for (final StructurePiece piece : start.getPieces()) {
            ...
            for (int z = az; z <= bz; ++z) for (int x = ax; x <= bx; ++x) {
                ...
                final BlockState fill = foundationMaterial(chunk.getBlockState(pos), desert);
                for (int y = supportY + 1; y < floorY; ++y) {
                    pos.set(x, y, z);
    ...
    static BlockState foundationMaterial(final BlockState support, final boolean desert) {
        if (desert) {
            if (support.is(Blocks.RED_SAND) || support.is(Blocks.RED_SANDSTONE)) {
                return Blocks.RED_SANDSTONE.defaultBlockState();
            }
            return Blocks.SANDSTONE.defaultBlockState();
        }
        if (support.is(Blocks.DEEPSLATE) || support.is(Blocks.TUFF)) {
            return Blocks.DEEPSLATE.defaultBlockState();
        }
        if (support.is(Blocks.GRANITE) || support.is(Blocks.DIORITE) || support.is(Blocks.ANDESITE)
            || support.is(Blocks.STONE) || support.is(Blocks.GRAVEL)) {
            return Blocks.STONE.defaultBlockState();
        }
        if (support.is(Blocks.DIRT) || support.is(Blocks.GRASS_BLOCK) || support.is(Blocks.COARSE_DIRT)
            || support.is(Blocks.PODZOL) || support.is(Blocks.ROOTED_DIRT) || support.is(Blocks.MUD)
            || support.is(Blocks.MYCELIUM)) {
