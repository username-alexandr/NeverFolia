...
if [ ! -d "${FOLIA_DIR}" ]; then echo "[NeverFolia] Folia worktree not found: ${FOLIA_DIR}" >&2; exit 2; fi
...
if grep -q 'key\.location()' "${HELPER_FILE}"; then echo "[NeverFolia] Failed to normalize ResourceKey API in placement helper" >&2; exit 3; fi
...
echo "[NeverFolia] Applying legacy R3 village availability base"
python3 "${ROOT_DIR}/scripts/tune-never-overworld-village-availability-r3.py" "${FOLIA_DIR}"
echo "[NeverFolia] Restoring strict R5/R6 village dry safety (9/9, radius 48)"
python3 "${ROOT_DIR}/scripts/tune-never-overworld-village-safety-r5.py" "${FOLIA_DIR}"
...
echo "[NeverFolia] Applying historical fixed-radius32 village envelope"
python3 "${ROOT_DIR}/scripts/tune-never-overworld-dense-village-radius32.py" "${FOLIA_DIR}"
echo "[NeverFolia] Sizing final village envelope to vanilla Jigsaw reach plus one chunk margin"
...
echo "[NeverFolia] Applying R9 bounded all-village locate policy after exact generation safety"
python3 "${ROOT_DIR}/scripts/harden-never-overworld-village-locate-r9.py" "${FOLIA_DIR}"
...
echo "[NeverFolia] Applying field-R6 buried ore density reduction"
python3 "${ROOT_DIR}/scripts/tune-never-overworld-ore-field-r6.py" "${FOLIA_DIR}"
echo "[NeverFolia] Applying field-R7 screenshot-driven buried ore scarcity"
python3 "${ROOT_DIR}/scripts/tune-never-overworld-ore-field-r7.py" "${FOLIA_DIR}"
...
echo "[NeverFolia] Applying two-stage ore scarcity after CARVERS and FEATURES"
python3 "${ROOT_DIR}/scripts/apply-never-overworld-ore-exposure-prune.py" "${FOLIA_DIR}"
...
echo "[NeverFolia] Applying R8 large cross-chunk cavern flood connectivity"
python3 "${ROOT_DIR}/scripts/harden-never-overworld-flood-connectivity-r8.py" "${FOLIA_DIR}"
...
echo "[NeverFolia] Melting drowned frozen surface remnants (R8-C)"
python3 "${ROOT_DIR}/scripts/harden-never-overworld-frozen-surface-r8c.py" "${FOLIA_DIR}"
...
if ! compgen -G "${POST_PATCH_DIR}/*.patch" > /dev/null; then echo "[NeverFolia] No additional post-apply patch files"; exit 0; fi
cd "${FOLIA_DIR}"
echo "[NeverFolia] Applying post-apply patch files"
for patch_file in "${POST_PATCH_DIR}"/*.patch; do echo "  -> $(basename "${patch_file}")"; git apply --whitespace=nowarn "${patch_file}"; done
