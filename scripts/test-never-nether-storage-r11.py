...
class Contracts(unittest.TestCase):
    ...
    def test_prepare_readonly(self):
        before=self.snapshot();self.assertEqual(len(P.prepare(self.folia)),3);self.assertEqual(before,self.snapshot())
    ...
    def test_equal_installed_helper(self):
        p=self.folia/P.JAVA/P.HELPER;p.parent.mkdir(parents=True);p.write_text('synthetic helper');self.assertEqual(len(P.prepare(self.folia)),3)
