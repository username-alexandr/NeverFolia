...
for index in "${!TARGETS[@]}"; do
  ...
  ordinal=$((index + 1))
  token="NR_R9_VILLAGE_LOCATE_DONE_${ordinal}"
  ...
  segment="${TEST_DIR}/segment-${ordinal}.log"
  ...
  if grep -Eqi 'Could not find|Couldn.t find|Unknown registry|Unknown structure|Failed to execute|Incorrect argument' "${segment}"; then
    echo "[NeverFolia][R9 village locate] FAIL: no usable result for ${target}" >&2
    tail -n 120 "${segment}" >&2 || true
    exit 1
  fi
...
if ! cmp -s "${BEFORE}" "${AFTER}"; then
  echo '[NeverFolia][R9 village locate] FAIL: village/map-tag locate created persisted Anvil records.' >&2
  diff -u "${BEFORE}" "${AFTER}" >&2 || true
  exit 1
fi

if grep -Eqi 'FoliaWatchdogThread|has not responded in [0-9]' "${TEST_DIR}/server.log"; then
  echo '[NeverFolia][R9 village locate] FAIL: Folia watchdog signature exists in runtime log.' >&2
  grep -Ein 'FoliaWatchdogThread|has not responded in [0-9]' "${TEST_DIR}/server.log" >&2 || true
  exit 1
fi

python3 - "${RESULTS}" "${BEFORE}" "${AFTER}" "${REPORT}" <<'PY'
import json
import sys
from pathlib import Path

results_path, before_path, after_path, report_path = map(Path, sys.argv[1:])
results = []
for line in results_path.read_text(encoding='utf-8').splitlines():
    target, elapsed = line.split('\t', 1)
    results.append({'target': target, 'elapsed_seconds': int(elapsed)})
before = before_path.read_text(encoding='utf-8').splitlines()
after = after_path.read_text(encoding='utf-8').splitlines()
report = {
    'status': 'ok',
    'contract': 'R9 bounded all-village locate + cartographer map tags',
    'targets_checked': len(results),
    'command_timeout_seconds': 20,
    'max_elapsed_seconds': max((row['elapsed_seconds'] for row in results), default=0),
    'watchdog_signatures': 0,
    'anvil_records_before': len(before),
    'anvil_records_after': len(after),
    'new_anvil_records_from_locate': sorted(set(after) - set(before)),
    'results': results,
}
report_path.parent.mkdir(parents=True, exist_ok=True)
report_path.write_text(json.dumps(report, indent=2) + '\n', encoding='utf-8')
print('[NeverFolia][R9 village locate] PROOF:', json.dumps(report, sort_keys=True))
PY
...
echo '[NeverFolia][R9 village locate] ALL 5 VILLAGES + 5 MAP TAGS BOUNDED / ZERO-GENERATION / NO-WATCHDOG OK'
