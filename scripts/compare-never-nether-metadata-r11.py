...
def observations(root:Path,phase:str|None=None)->dict:
    ...
    for item in report['observations']:
        ...
        for y,s in enumerate(sections,-8):
            if (s['ChunkX'],s['SectionY'],s['ChunkZ'])!=(c[0],y,c[1]) or s['Seed']!=report['seed'] or s['Schema']!=1 or s['Profile']!='NN-R11-SUBSTRATE-1-REMOTE-R10-PRIORITY':raise ValueError('Metadata world identity mismatch')
...
def compare(left:Path,right:Path,phase:str|None=None)->dict:
    a=observations(left,phase);b=observations(right,phase)
    ...
    return {'schema':1,'gate':'R11-exact-block-and-substrate-observations','observations':len(a['records']),'sections':len(a['records'])*64,'differences':differences,'passed':not differences,'release_ready':False,'scope':'Exact checked block and metadata snapshot bytes, not every chunk/entity or proof of identical request history.'}

def main()->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--left-observations',type=Path,required=True);p.add_argument('--right-observations',type=Path,required=True);p.add_argument('--phase',choices=('carvers','light','settled'));p.add_argument('--output',type=Path,required=True);a=p.parse_args()
    try:r=compare(a.left_observations,a.right_observations,a.phase)
