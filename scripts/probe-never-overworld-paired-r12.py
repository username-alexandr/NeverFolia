...
def generate(root, observer, nbt, jar, packs, out, label, source_sha, plan=None):
    ...
    evidence={'label':label,'source_sha':source_sha,'seed':SEED,'jar_sha256':observer.sha(jar),
              'pack_sha256':hashes,'phases':[], 'areas':[] if plan is None else json.loads(json.dumps(plan))}
    ...
    def phase(name, action):
        ...
        try:
            ...
            item['exit_code']=server.stop(); item['normal_stop']=True
    ...
    if plan is None:
        ...
        def initial(server):
            for target,origin in zip(observer.FORESTS,((0,0),(12000,0),(-12000,0))):
                ...
                server.load_dimension(chunks,DIMENSIONS[0])
            ...
            for target in MINE_TARGETS:
                ...
                chunks=[(a,b) for b in range(cz-2,cz+3) for a in range(cx-2,cx+3)]
                server.load_dimension(chunks,DIMENSIONS[0]); initials.append((target,x,z,chunks))
    ...
    def complete(server):
        for area in evidence['areas']:
            server.load_dimension(area['chunks'],DIMENSIONS[0],5 if area['kind']=='mine' else 0)
        server.load_dimension([(x,z) for x in range(-196,-193) for z in range(-394,-391)],DIMENSIONS[1])
    second=phase('complete',complete)
    # Reopen candidate and repeat loaded coverage, with fluid ticks, to detect
    # lost persisted protection and immediate flooding after a restart.
    if label=='candidate': second=phase('restart',complete)
    result=[]; forest_roots={}
    for area in evidence['areas']:
    ...
    observer.write_json(out/(label+'-plan.json'),evidence)
    return evidence,resources(observer.Volume(forest_roots))
...
def main():
    ...
    report={'schema':1,'seed':SEED,'source_sha':a.source_sha,'baseline_sha':BASE_SHA,'production_ready':False,
    ...
    try:
        ...
        report['paired_gate_pass']=report['ore']['pass'] and report['dry_mines_pass'] and report['tree_observation']['reduced_in_sample']
