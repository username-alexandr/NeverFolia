...
def recheck_block(kind: str) -> str:
    tail = "level, state.randomState()" if kind == "fast" else "heightAccessor, randomState"
    reject = '''
                        debugVillage(id, "R9V16B_RELAXED_EXACT_REJECT", chunkPos, centerSurfaceY,
                            "sample=" + probe[0] + "," + probe[1] + ",solidBase=" + solidBase
                                + ",rings=" + MAX_CANDIDATE_RINGS);''' if kind == "fast" else ""
            ...
            if ("minecraft:village_snowy".equals(id) || "minecraft:village_taiga".equals(id)) {{
                for (final int[] probe : preliminaryProbes) {{
                    final int solidBase = generator.getBaseHeight(
                        centerX + probe[0], centerZ + probe[1], Heightmap.Types.OCEAN_FLOOR_WG, {tail}
                    );
...
def patch_method(text: str, signature: str, kind: str) -> str:
    start, end = method_bounds(text, signature); method = text[start:end]
    if V16B_MARKER in method: return text
    if V16_MARKER not in method: fail(f"{kind}: V16 marker missing")
    if method.count(INSERT_ANCHOR) != 1: fail(f"{kind}: exact-mask anchor count mismatch")
...
def validate(fast: str, policy: str) -> None:
    for label, text, sig in (("fast", fast, FAST_SIG), ("generation", policy, POLICY_SIG)):
        ...
        if V16B_MARKER not in method: fail(f"{label}: V16b marker missing")
        if method.count("for (final int[] probe : preliminaryProbes)") < 2: fail(f"{label}: relaxed recheck loop missing")
        if "Heightmap.Types.OCEAN_FLOOR_WG" not in method: fail(f"{label}: solid heightmap missing")
        if "Structure.generate(" in method or "NeverOverworldGeneratedVillageSafety.preview" in method: fail(f"{label}: generated preview leaked")
        if label == "generation":
            a = method.find(V16B_MARKER); b = method.find(NON_VILLAGE_ANCHOR, a)
            if b < 0: fail("generation: non-village boundary missing")
            if "WORLD_SURFACE_WG" in method[a:b]: fail("generation: WORLD_SURFACE_WG leaked into village recheck")
...
def self_test() -> None:
    ...
    fast = patch_method(fast, FAST_SIG, "fast"); policy = patch_method(policy, POLICY_SIG, "generation")
    validate(fast, policy)
    print("[NeverFolia][R9 village variant mask v16b] SELF-TEST OK")


def main() -> None:
    ap = argparse.ArgumentParser(); ap.add_argument("folia", nargs="?", type=Path); ap.add_argument("--self-test", action="store_true"); args = ap.parse_args()
    ...
    fast = patch_method(fp.read_text(encoding="utf-8"), FAST_SIG, "fast"); policy = patch_method(pp.read_text(encoding="utf-8"), POLICY_SIG, "generation")
    validate(fast, policy); fp.write_text(fast, encoding="utf-8"); pp.write_text(policy, encoding="utf-8")
