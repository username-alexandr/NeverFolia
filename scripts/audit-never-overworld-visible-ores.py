...
def audit(world: Path, max_chunks: int, min_y: int, max_y: int) -> dict:
    ...
    for cx, cz in A.generated_chunks(region):
        ...
        # Only the 14x14 interior is used. This makes every horizontal neighbour
        # lookup stay within this persisted chunk, so exposure statistics never
        # depend on whether an adjacent region/chunk happened to be generated.
...
def self_test() -> None:
    ...
    print("  persisted metric: total vs air/fluid-exposed ore in interior FULL-chunk columns")
